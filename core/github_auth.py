...
class GitHubOAuthFlow:
    ...
    def authenticate(self) -> Optional[str]:
        ...
        if not self.CLIENT_ID or not self.CLIENT_SECRET:
            ...
            click.echo(
                "   - Homepage URL: https://github.com/your-username/ce-lib-wizard", err=True
            )
        ...
        response = requests.post(
            self.TOKEN_URL, headers={"Accept": "application/json"}, data=token_data
        )
...
def get_github_token_via_gh_cli() -> Optional[str]:
    ...
    try:
        result = run_command(
            ["gh", "auth", "token"], clean_env=False  # Keep full environment to preserve PATH
        )
